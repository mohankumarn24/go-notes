...
func main() {
	...
	fmt.Println("Index(\"m\"):", strings.Index(fullName, "m"))
	fmt.Println("Index(\"rio\"):", strings.Index(fullName, "rio"))
	fmt.Println("Index(\"Sachin\"):", strings.Index(fullName, "Sachin"))
	fmt.Println("LastIndex(\"a\"):", strings.LastIndex(fullName, "a"))

	afterSplit := strings.Split(fullName, " ")
	afterJoin := strings.Join(afterSplit, " ")
	fmt.Println("Split():", afterSplit)
	fmt.Println("Join():", afterJoin)

	fmt.Println("Count(\"a\"):", strings.Count(fullName, "a"))
	fmt.Println("ToUpper():", strings.ToUpper(fullName))
	fmt.Println("ToLower():", strings.ToLower(fullName))
	fmt.Println("Compare(\"abc\", \"xyz\"):", strings.Compare("abc", "xyz"))
	fmt.Println("Fields():", strings.Fields("  a  b   c "))
	fmt.Println("Repeat():", strings.Repeat("*", 10))

	fmt.Println("Replace():", strings.Replace(fullName, "a", "@", 1))
	fmt.Println("ReplaceAll():", strings.ReplaceAll(fullName, "Emporio", "Giorgio"))

	fmt.Println("TrimSpace():", strings.TrimSpace("  Hello  "))
	fmt.Println("TrimPrefix():", strings.TrimPrefix(fullName, "Emp"))
	fmt.Println("TrimSuffix():", strings.TrimSuffix(fullName, "Armani"))
	...
	fmt.Println("string concat using strings.Builder:", builder.String())

	// 2. + operator (most common for a few strings)
	firstName2 := "Emporio"
	lastName2 := "Armani"
	fullName2 := firstName2 + " " + lastName2
	fmt.Println("string concat using +:", fullName2)

	// 3. += operator (append to an existing string)
	name3 := "Emporio"
	name3 += " Armani"
	fmt.Println("string concat using +=:", name3)
